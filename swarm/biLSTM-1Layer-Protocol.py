...
def _artifact_paths(output: str | None, primary_type: str) -> tuple[str, str]:
    if output:
        output_dir = os.path.dirname(output)
        stem = os.path.splitext(os.path.basename(output))[0]
        prefix = os.path.join(output_dir, stem) if output_dir else stem
        return f"{prefix}_scaler.joblib", f"{prefix}_split_manifest.json"
    return f"scaler_{primary_type}.joblib", "model_split_manifest.json"
...
if __name__ == "__main__":
    ...
    scaler_path, manifest_path = _artifact_paths(args.output, attack_id)
    manifest = {
    ...
    callbacks = [
        ReduceLROnPlateau(
            monitor="val_auc", factor=0.5, patience=3, min_lr=1e-6, mode="max", verbose=1
        ),
        EarlyStopping(
            monitor="val_auc", patience=6, restore_best_weights=True, mode="max", verbose=1
        ),
    ]
...
""")
    ...
    print(f"\n── biLSTM model ──")
    print(f"Trained on per-window labels, primary type '{args.primary_type}'")

    model.save(model_name)
    print(f"\n✓ biLSTM model saved → {model_name}")
